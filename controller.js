  ...
  loop((t, dt) => {
    for (var i in state) {
      if (state["_" + i] != undefined) {
        state["_" + i] +=
          (state["_e_" + i] || 0.1) * (state[i] - state["_" + i]);
        state["_" + i] =
          Math.abs(state[i] - state["_" + i]) < 0.0001
            ? state[i]
            : state["_" + i];
      }
    }
  ...
  loop((t, dt) => {
    if (window.explode > 0) {
      ...
      for (var i = 0; i < 500; i++) {
        ...
        q.sprite.width = q.sprite.height = q.sprite.sz * 20 + 5;
...
//controller
{
  ...
  loop(t => {
    ...
    if (
      ...
      controlstate.ball_enabled &&
      !controlstate.launched &&
      !controlstate.selecting_power &&
      controlstate.angle_selected
    ) {
    ...
    bar_direction_hint.alpha =
      (Math.sin(t * 3) * 0.5 + 0.5) *
      controlstate._hint_angle *
      controlstate._angle_enabled;
    bar_direction_touchhint.alpha =
      (Math.sin(t * 10) * 0.2 + 0.6) *
      controlstate._hint_angle *
      controlstate._angle_enabled;
    ...
    text_direction_start.alpha =
      controlstate._inactive * (Math.sin(t * 10) * 0.2 + 0.8);
